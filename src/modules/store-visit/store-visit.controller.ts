...
export class StoreVisitController {
  ...
  previousRoutesAssigned = async (
    ...
    try {
      ...
      const result =
        await this.storeVisitService.getPreviousAssignRoute(user_id);
      res.status(200).json({
  ...
  assignPreviousRoutes = async (
    ...
    try {
      ...
      const result = await this.storeVisitService.usePreviousAssignedRoutes(
        user_id,
        visit_date,
      );
      res.status(200).json({
